        ...
        switch lower(parameter)
            ...
            case 'location'
                if ~(numel(value)==2 && isnumeric(value)) && ...
                        isempty(strcmpi(value,directions))
...
delete(findobj(hAxes,'tag','scalebar'));
...
text(linepos(2,1),linepos(2,2),0,[num2str(scalelength*S) unitstring],'verticalalignment',textalignment{1},'horizontalalignment',textalignment{2}, 'color', colour, 'fontweight', fontweight, 'parent', hg);
...
udata = {'ScaleLengthRatio',scalelengthratio;...
    ...
    'Listeners',hL;...
    'Bold',boldflag;...
    'Unit',unitstring(2:end);...
    'UnitScale',S};
set(hg,'UserData',udata);

% CALLBACK FUNCTIONS
function deleteScaleBar(src,~)
udata = get(src,'UserData');
delete(udata{strcmpi(udata(:,1),'Listeners'),2});

function resetScaleBar(~,~,SB)

if ~isvalid(SB); return; end
%if strcmp(get(panH(1),'state'),'on'); return; end
...
UnitScale = udata{strcmpi(udata(:,1),'UnitScale'),2};

scalebar('hAxes',hAxes,'ScaleLengthRatio',scalelengthratio,'Location',location,'Colour',colour, 'Bold', boldflag,'Unit',Unit,'UnitScale',UnitScale);
