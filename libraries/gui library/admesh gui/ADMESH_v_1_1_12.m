...
if ~strcmp(ver,'2014b')
    errordlg('You need Matlab 2014b or newer in order to use ADMESH.','ADMESH')
end
...
%uiextras.set( gui.Window, 'DefaultBoxPanelTitleColor', get(0,'DefaultUicontrolBackgroundColor'));
%--------------------------------------------------------------------------

% Set default text type----------------------------------------------------
%uiextras.set( gui.Window, 'DefaultFixedWidthFontName', get(0,'FixedWidthFontName'));
%--------------------------------------------------------------------------
...
% Top box
mainLayout = uix.HBox('Parent', Layout, 'Spacing', 5 );
...
gui.ViewPanel = uiextras.Panel('Parent', mainLayout,'Padding',15);
...
controlLayout = uiextras.VBox( 'Parent', controlPanel,'Padding', 15, 'Spacing', 20 );

% + Create panel for settings
settingsPanel = uiextras.Panel( 'Parent', controlLayout,'Title', 'Mesh Criteria');
...
% Set proportion ratio
set( controlLayout, 'Sizes', [-1 30] ); % Make the list fill the space

% Initialize grid for seperating criteria
g = uiextras.Grid( 'Parent', settingsPanel, 'Spacing', 10 ,'Padding',10);
...
uicontrol('style','text','string','Elevation:','HorizontalAlignment', 'center','parent',g)
...
% Set the sizes
set( g, 'ColumnSizes', [-1 -1 -1], 'RowSizes', [25 25 25 25 25 25 25 25 25]  );
